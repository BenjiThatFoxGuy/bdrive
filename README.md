...
- **Changed: "Show in Folder" now navigates to the file's actual folder instead of a generic browse-view jump.** Previously, "Show in folder" from search/recent results only resolved a `parentId` and jumped into a generic browse view with no real path context. The backend now computes and returns each file's full path on file-listing endpoints (previously the schema had a `path` field that was never populated — `pkg/services/file_query_builder.go` now selects it via the existing `get_path_from_file_id` SQL function), and the UI uses that path to navigate directly to the file's real folder and highlight it there. This is controlled by a new "Use path navigation for 'Show in folder'" setting (enabled by default), which falls back to the original browse-view jump when disabled. For security, "Show in folder" is hidden from share recipients viewing someone else's shared list. ([bdrive#fab018d](https://github.com/BenjiThatFoxGuy/bdrive/commit/fab018d279302b69c577fa8900cee960febb45e3), [bdrive-ui#919b941](https://github.com/BenjiThatFoxGuy/bdrive-ui/commit/919b9415923daed23d42db05e63109926b4712e0), [bdrive-ui#7c20baa](https://github.com/BenjiThatFoxGuy/bdrive-ui/commit/7c20baaa3e127bf35cbbe25ec446e369b29036db))
