  ...
  ({
    ...
    return (
      <Link
        ...
        className="group relative flex flex-col gap-4 p-5 bg-surface border border-border/50 rounded-[28px] hover:bg-surface transition-all duration-300"
      >
        <div className="flex justify-between items-start">
          <div className="p-3 rounded-2xl bg-accent-soft text-accent-soft-foreground transition-transform group-hover:scale-110 duration-300">
            <Icon className="size-6" />
          </div>
          <div className="text-right">
            <p className="text-sm font-medium text-muted capitalize">{category}</p>
            <p className="text-lg font-bold text-foreground">{totalFiles} Files</p>
          </div>
        </div>
        ...
        <div className="mt-2">
          ...
          <div className="mt-3 h-2 w-full bg-surface-secondary rounded-full overflow-hidden">
            <div
...
export const StorageView = memo(() => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-surface rounded-[32px] p-6 md:p-8 border border-border/50">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="p-4 rounded-[24px] bg-accent text-accent-foreground">
              <IcOutlineStorage className="size-8" />
            </div>
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-foreground">Total Storage</h2>
              <p className="text-muted font-medium">Overview of your drive usage</p>
            </div>
          </div>
          <div className="flex gap-8 md:gap-12">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-muted">Total Files</span>
              <span className="text-3xl font-bold text-accent">{totalStats.totalFiles}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-12 bg-surface rounded-[32px] p-6 border border-border/50">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-2 rounded-xl bg-accent-soft text-accent-soft-foreground">
              <IcOutlineDataset className="size-6" />
            </div>
            <h3 className="text-xl font-bold text-foreground">Upload Activity</h3>
          </div>
