...
import { ColorsLight, FbIcon, useIconData } from "file-browser";
    ...
    <div className={clsx("group/item w-full transition-all duration-300", isChild && "pl-4")}>
      <div
        className={clsx(
          ...
          "hover:bg-surface-tertiary group-data-[hover=true]:bg-surface-tertiary",
          status === FileUploadStatus.UPLOADING && "bg-accent-soft/50",
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="flex items-center gap-2">
            ...
            {isFolder && childFiles.length > 0 && (
              <span className="text-xs px-2 py-0.5 bg-surface-secondary text-surface-secondary-foreground rounded-full">
                {childFiles.length}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 text-xs text-muted">
            ...
            {status === FileUploadStatus.UPLOADING && (
              <span className="text-accent font-medium animate-pulse text-[10px] uppercase tracking-wider">
                Uploading...
              </span>
            )}
            {status === FileUploadStatus.FAILED && (
              <span className="text-danger font-medium text-[10px] uppercase tracking-wider">
                Failed
              </span>
            )}
          </div>
      ...
      {isFolder && !collapsed && childFiles.length > 0 && (
        <div className="relative mt-1">
          <div className="space-y-0.5 border-l border-border/30 ml-5 pl-1">
