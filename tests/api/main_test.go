...
func TestMain(m *testing.M) {
	...
	if os.Getenv("TEST_DATABASE_URL") == "" && os.Getenv("DATABASE_URL") == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "tests/api: TEST_DATABASE_URL not set and cannot start test DB container: %v\n", err)
			fmt.Fprintf(os.Stderr, "tests/api: set TEST_DATABASE_URL or ensure Docker is available\n")
